...
category: java
tags: [java]
keywords: DES、AES、RSA、对称加密、非对称加密、公钥、私�?
---
...
`2.` 计算字符`h`到`e`的偏移量，值为3，则表示原文偏移�?个位置�?
...
`2.` 短文可能严重偏离标准频率，假如文章少�?00个字母，那么对它的解密就会比较困难，而且不是所有文章都适用标准频率�?
...
例如，对十进制数�?2345678循环右移1位的结果�?1234567�?
...
也可以简单的理解为，参与异或运算的两数位相等，则结果�?，不等则�?�?
...
`DES`是`Data Encryption Standard`(数据加密标准)的缩写。它是由`IBM`公司研制的一种对称加密算法，美国国家标准局�?977年公布把它作为非机要部门使用的数据加密标准，三十年来，它一直活跃在国际保密通信的舞台上，扮演了十分重要的角色�?
...
经过`16`轮迭代后，左、右半部分合在一起经过一个末置换（数据整理）这样就完�?了加密过程�?
...
比如字符串`ABC`,其在计算机内存中的十六进制表示为41�?2�?3，十进制表示�?5�?6�?7二进制表示为�?

`01000001`	`01000010`	`01000011`   将这三个二进制数依次取`6bit`

`010000/01`	`0100/0010`	`01/000011` 就转换成�?

`010000`  `010100`  `001001`  `000011`，将这四个二进制数转换成十进制数为：16�?0�?�?�?
...
比如字符串`ce`，其在内存中的十六进制表示为63�?5;十进制表�?9�?01；二进制表示为：

`01100011`	`01100101`  依此�?bit

`011000/11`	`0110/0101`  这时，第`3`个字符不足`6`位，在后面补零，也就是`0101`变成了`010100`。转换结果为

`011000`  `110110`	`010100` 这`3`个二进制数转换成十进制为24�?4�?0。对照码表得出结果`Y2U`。编码后每补两个零就用一个`=`填充，最后编码得出`Y2U=`�?

*`c、`当余数为1�?

比如字符串`{`，其在内存中的十六进制表示为`$78`，十进制�?23，二进制位表示为�?

`01111011` 依此�?bit

`011110/11 ` 补零后为

`011110/110000`  转换结果为`011110`和`110000`。这两个二进制数转换成十进制数为 30�?8。对照码表得出结果为`ew`，补上`=`，最后编码得出`ew==`。解码也很简单，是编码的逆过程，即将每个字符�?
...
DES的安全性首先取决于密钥的长度。密钥越长，破译者利用穷举法搜索密钥的难度就越大。DES采用64bit分组长度�?6bit密钥长度�?

密钥大小（bit）| 密钥个数 | 每微妙执行一次解密所需要的时间 | 每微妙执行一百万次解密所需要的时间
------------- | ---------| ----------------------- | -------------------------------
  56          | 2^56=7.2*10^16 |  2^55µm =1142�?| 1001小时
...
例如后来演变出的`3-DES`算法使用�?个独立密钥（密钥长度�?68bit）进行三重加密，这就比`DES`大大提高 了安全性，如果`56`位`DES`用穷举搜索来破译需�?$2^56$$次运算，而`3-DES`则需�?^112次�?

### 2.AES算法

3DES缺陷是算法运行行对较慢。因为原来DES是为70年代的硬件设计的，算法代码并不高效，�?DES是DES算法�?轮迭代，因此更慢。而且DES�?DES的分组大小都�?4bit�?DES密钥长度却是168bit，处于加密效率和安全的考虑，需要更大的分组长度。于是AES应运而生�?

Advanced Encryption Standard高效加密标准，该标准是美国国家标准技术研究所�?001年颁布的，AES旨在取代DES成为广泛使用的标准，2006年AES已成为最流行的对称加密算法�?

AES使用的分组大小为128bit，密钥长度可以为128�?92�?56bit。最简单最常用的也就是128bit�?6字节）的密钥�?

**AES算法过程�?*

AES加密过程涉及�?种操做：字节替代（SubBytes）、行移位（ShiftRows）、列混淆（MixColumns）和轮密钥加（AddRoundKey）�?

解密过程分别为对应的逆过程。由于每一步操做都是可逆的，按照相反的顺序进行解密即可恢复明文。加解密中每轮的密钥分别由初始密钥扩展得到。算法中16（byte）字节的明文、密文和轮密钥都以一�?*4的矩阵表示�?
...
step1：随机选取两个数`p`、`q`，满足互�?

Step2：`n = p * q`, //公开模数Public Modules ，其二进制位数即为密钥长度�?

Step3：`𝑔= ɐ (p) ɐ(q) = (p-1)*(q-1)` //欧拉函数

Step4：在`1`和`𝑔`之间任意一个整数`e`，满�?`1 < e < 𝑔` //Public Exponent ,公开旨数

Step5:	由`e * d mode g = 1` 关系式推导出�?`d`, //Private Exponent ,私有指数
